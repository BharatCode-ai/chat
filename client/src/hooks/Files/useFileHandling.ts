...
const useFileHandlingCore = (params: UseFileHandling | undefined, fileState: FileHandlingState) => {
  ...
  const shouldUseContextUpload = useCallback(
    (file: File) => {
      if (!isConversationUpload || isAssistantsEndpoint(endpointType ?? endpoint)) {
        return false;
      }
      ...
      return Boolean(
        checkType(fileType, fileConfig.text?.supportedMimeTypes || []) ||
          checkType(fileType, fileConfig.ocr?.supportedMimeTypes || []) ||
          checkType(fileType, fileConfig.stt?.supportedMimeTypes || []),
      );
    },
    [endpoint, endpointType, fileConfig, isConversationUpload],
  );
  ...
  const handleFiles = async (_files: FileList | File[], _toolResource?: string) => {
    ...
    try {
      ...
      const hasAutomaticContextFiles =
        _toolResource == null && fileList.some((file) => shouldUseContextUpload(file));
      ...
      filesAreValid = validateFiles({
        ...
        fileConfig,
        endpointFileConfig,
        toolResource: hasAutomaticContextFiles ? EToolResources.context : _toolResource,
      });
    } catch (error) {
    ...
    for (const originalFile of fileList) {
      ...
      try {
        const toolResource =
          _toolResource ?? (shouldUseContextUpload(originalFile) ? EToolResources.context : '');
